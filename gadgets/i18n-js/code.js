...
(function (window, $, mw, undefined) {
	 ...
	 *     https://dev.miraheze.org/wiki/<page_title>/i18n.json
	 *
     * @param {object} options Options set by the loading script:
     * - {string} apiEntrypoint: e.g. https://meta.miraheze.org/w/api.php
	 *     Set as 'self' to fetch the i18n from your own wiki instead.
    ...
    function loadMessages(name, options) {
        ...
        var deferred = $.Deferred(),
            apiEntrypoint = 'https://dev.miraheze.org/w/api.php',
            page = name + '/i18n.json',
            params, 
			api;

        options = options || {};
        options.apiEntrypoint = options.apiEntrypoint || apiEntrypoint;
        options.page = page;
		...
		if (options.apiEntrypoint === 'self' || options.apiEntrypoint.startsWith(conf.wgServer)) {
			// Own wiki
			api = new mw.Api();
		} else {
			api = new mw.ForeignApi(options.apiEntrypoint, { anonymous: true });
		}
			...
			api.get(params)
				.done(function (data) {
					...
					if (revisionData) {
						...
						console.error(
							'Failed to fetch contents from ' + options.apiEntrypoint + ' for page ' + page, 
							data.query.pages[data.query.pageids[0]].missing !== undefined ? 'Page not found' : 
								(data.warnings || data.error || 'Unknown error')
						);
						deferred.resolve();
					}
				})
